...
class Survey(Base):
    ...
    created_by = Column(Integer, ForeignKey('users.id'), nullable=True) #3
...
class Option(Base):
    ...
    id = Column(Integer, primary_key=True) #0
    survey_id = Column(Integer, ForeignKey('surveys.id'), nullable=False) #1
...
class Vote(Base):
    ...
    id = Column(Integer, primary_key=True) #0
    survey_id = Column(Integer, ForeignKey('surveys.id'), nullable=False) #1
    user_id = Column(Integer, ForeignKey('users.id'), nullable=True) #2
...
class VoteOption(Base):
    ...
    id = Column(Integer, primary_key=True) #0
    vote_id = Column(Integer, ForeignKey('votes.id'), nullable=False) #1
    option_id = Column(Integer, ForeignKey('options.id'), nullable=False) #2
